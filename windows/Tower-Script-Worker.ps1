...
while($true){
    try{
        ...
        if($null -ne $response.job){
            ...
            try{
                [IO.File]::WriteAllText($inputPath,('$ErrorActionPreference = ''Stop''' + "`r`n" + [string]$job.body),(New-Object Text.UTF8Encoding($true)))
                $process=Start-Process -FilePath "$env:SystemRoot\System32\WindowsPowerShell\v1.0\powershell.exe" -ArgumentList @('-NoProfile','-ExecutionPolicy','Bypass','-File',('"{0}"' -f $inputPath)) -WindowStyle Hidden -WorkingDirectory $env:USERPROFILE -RedirectStandardOutput $outputPath -RedirectStandardError $errorPath -PassThru
                ...
                else{$process.WaitForExit();$ok=$process.ExitCode -eq 0;$message=if($ok){'Script completed'}else{"Script failed (exit $($process.ExitCode))"}}
